...
class JdAzwSpider(CrawlSpider):
    ...
    rules = (
        ...
        Rule(SgmlLinkExtractor(allow=("http://m\.jd\.com/comment/*",)),callback='parse_comment'),
        Rule(SgmlLinkExtractor(allow=("http://m\.jd\.com/ware/comments*",)),callback='parse_ware'),
    )
    ...
    def parse_products(self,response):
        ...
        #get the next page
        next_page_url = hxs.select('//div[@class="page"]/a[0]/@href').extract()[0]
    ...
    def parse_product(self,response):
        ...
        comment_url = hxs.select('//div[@class="content content2"]//a[2]').extract()[0]
    ...
    def parse_comment(self,response):
        ...
        hxs = HtmlXPathSelector(response)
        proid_pattern =  re.compile('([0-9]+)\.html')
    ...
    def parse_ware(self,response):
        ...
        for comment in comments:
            ...
            product_item['score'] = comment.select('div[@class="u-score"]/span/text()').extract()
            product_item['comment'] = comment.select('div[@class="u-summ"]/text()').extract()
